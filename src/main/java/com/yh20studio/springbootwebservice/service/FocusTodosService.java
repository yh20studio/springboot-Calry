...
public class FocusTodosService {
    ...
    public List<FocusTodosMainResponseDto> getNotSuccessFocusTodos() {
        ...
        return focusTodosRepository.findNotSuccessByMemberId(memberId)
            .map(FocusTodosMainResponseDto::new).collect(Collectors.toList());
    ...
    public Long delete(Long id) {
        FocusTodos focusTodos = focusTodosRepository.findById(id)
            .orElseThrow(
                () -> new RestException(HttpStatus.NOT_FOUND, "해당 FocusTodos 값을 찾을 수 없습니다."));
