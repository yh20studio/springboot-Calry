...
public class MemberService {
    ...
    public Long signup(MemberSaveRequestDto memberSaveRequestDto) {
        ...
        List<LabelColors> labelColorsList = labelColorsRepository.findAllWithoutHoliday().collect(Collectors.toList());
    ...
    public AccessTokenResponseDto login(MemberSaveRequestDto memberSaveRequestDto) {
        ...
        UsernamePasswordAuthenticationToken authenticationToken = Member.toAuthentication(memberSaveRequestDto.getEmail(), memberSaveRequestDto.getPassword());
        ...
        if(refreshTokenRepository.existsByMember(member)){
            ...
            refreshToken.updateWhole(tokenDto.getRefreshToken(), tokenDto.getRefreshTokenExpiresIn());
        }
        else{
    ...
    public MessageResponse logout(AccessTokenRequestDto accessTokenRequestDto) {
        ...
        if (!jwtUtil.validateToken(accessTokenRequestDto.getAccessToken())) {
            throw new RestException(HttpStatus.UNAUTHORIZED, "Access Token이 유효하지 않습니다."); //401 Error
        }
        ...
        RefreshToken refreshToken = refreshTokenRepository.findByMember(member)
                .orElseThrow(() -> new RestException(HttpStatus.UNAUTHORIZED, "로그아웃 된 사용자입니다.")); //401 Error
    ...
    public AccessTokenResponseDto reissueAccessToken(AccessTokenRequestDto accessTokenRequestDto) {
        ...
        RefreshToken refreshToken = refreshTokenRepository.findByMember(member)
                .orElseThrow(() -> new RestException(HttpStatus.NOT_FOUND, "Refresh Token을 찾을 수 없습니다.")); //401 Error
    ...
    public MemberMainResponseDto getMyInfo() {
        return memberRepository.findById(securityUtil.getCurrentMemberId())
                ...
                .orElseThrow(() -> new RestException(HttpStatus.NOT_FOUND, "로그인 유저 정보가 없습니다.")); // 404 error
    }
}
