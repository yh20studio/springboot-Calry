...
public class TodayRoutinesService {
    ...
    public TodayRoutinesMainResponseDto save(TodayRoutinesSaveRequestDto dto) {
        ...
        TodayRoutinesGroups todayRoutinesGroups = todayRoutinesGroupsRepository
            .findByMemberAndDate(memberId, LocalDate.parse(dto.getDate()))
            .orElseGet(() -> todayRoutinesGroupsRepository.save(
                (new TodayRoutinesGroupsSaveRequestDto(dto.getDate(), 0, 0, member)).toEntity()));
    ...
    @Transactional
    public List<TodayRoutinesMainResponseDto> saveList(
        TodayRoutinesSaveListRequestDto dtoSaveList) {
        Long memberId = securityUtil.getCurrentMemberId();
        Member member = memberRepository.findById(memberId)
            .orElseThrow(() -> new RestException(HttpStatus.UNAUTHORIZED, "잘못된 사용자 입니다."));

        TodayRoutinesGroups todayRoutinesGroups = todayRoutinesGroupsRepository
            .findByMemberAndDate(memberId, LocalDate.parse(dtoSaveList.getDate()))
            .orElseGet(() -> todayRoutinesGroupsRepository.save(
                (new TodayRoutinesGroupsSaveRequestDto(dtoSaveList.getDate(), 0, 0, member))
                    .toEntity()));
    ...
    public Long delete(Long id) {
        TodayRoutines todayRoutines = todayRoutinesRepository.findById(id)
            .orElseThrow(
                () -> new RestException(HttpStatus.NOT_FOUND, "해당 TodayRoutines 값을 찾을 수 없습니다."));
